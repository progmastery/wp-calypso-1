...
describe( 'Interval', function() {
	...
	describe( 'Running actions', function() {
		...
		it( 'Should add the given action', function() {
			...
			mount( <Interval onTick={ noop } period={ EVERY_SECOND } /> );
		...
		it( 'Runs onTick()', function() {
			const o = { counter: 0 };
			mount( <Interval onTick={ nudgeObject( o, 1 ) } period={ EVERY_SECOND } /> );
		...
		it( 'Changes the callback on prop changes', function() {
			const o = { counter: 0 };
			const wrapper = mount( <Interval onTick={ nudgeObject( o, 1 ) } period={ EVERY_SECOND } /> );
		...
		it( 'Adds the action when mounted', function() {
			...
			wrapper.setProps( { children: <Interval onTick={ nudgeObject( o, 1 ) } period={ EVERY_SECOND } /> } );
		...
		it( 'Removes the action when unMounted', function() {
			const o = { counter: 0 };
			const wrapper = mount( <div><Interval onTick={ nudgeObject( o, 1 ) } period={ EVERY_SECOND } /></div> );
